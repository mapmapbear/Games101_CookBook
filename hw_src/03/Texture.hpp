...
public:
  ...
  Eigen::Vector3f getColorBilinear(float u, float v) {
    ...
    auto left_bom = image_data.at<cv::Vec3b>(v_max, u_min);
    auto right_bom = image_data.at<cv::Vec3b>(v_max, u_max);

    float rs = (u_img - u_min) / (u_max - u_min);
    float rt = (v_img - v_max) / (v_min - v_max);

    auto cBot = (1 - rs) * left_bom + rs * right_bom;
    auto cTop = (1 - rs) * left_top + rs * right_top;
