...
static bool insideTriangle(double x, double y, const Vector3f *_v) {
  ...
  return AB[0] * AP[1] - AB[1] * AP[0] > 0 &&
         BC[0] * BP[1] - BC[1] * BP[0] > 0 && CA[0] * CP[1] - CA[1] * CP[0] > 0;
}
  ...
  if (MSAA) {
    ...
    for (int x = min_x; x <= max_x; ++x) {
      for (int y = min_y; y <= max_y; ++y) {
        ...
        int count = 0;
        for (int i = 0; i < 4; ++i) {
          if (insideTriangle(static_cast<float>(x) + pos[i][0],
                             static_cast<float>(y) + pos[i][1], t.v)) {
            auto [alpha, beta, gamma] =
                computeBarycentric2D(static_cast<float>(x) + pos[i][0],
                                     static_cast<float>(y) + pos[i][1], t.v);
            float w_reciprocal =
        ...
        if (count) {
          if (depth_buf[get_index(x, y)] > minDepth) {
            Vector3f color = t.getColor() * (float)count / 4.0f;
    ...
    for (int x = min_x; x <= max_x; ++x) {
      for (int y = min_y; y <= max_y; ++y) {
        if (insideTriangle(static_cast<float>(x) + 0.5f,
                           static_cast<float>(y) + 0.5f, t.v)) {
          auto [alpha, beta, gamma] = computeBarycentric2D(
              static_cast<float>(x) + 0.5f, static_cast<float>(y) + 0.5f, t.v);
          float w_reciprocal =
              1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
          float z_interpolated = alpha * v[0].z() / v[0].w() +
                                 beta * v[1].z() / v[1].w() +
